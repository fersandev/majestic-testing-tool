...
if($unitMonitor['status'] == 'ok') {
	...
	if($unitMonitorInfo['implementingType'] == 'php') {
		...
		if($unitMonitorInfo['typeValueExpected'] == 'boolean') {
...
<br>
$curl = curl_init();curl_setopt_array($curl, array(CURLOPT_URL => $_SERVER[\'SERVER_NAME\']."/vendor/fersandev/majestic-testing-tool/mttphp.php?flag=true&json=".json_encode(array(\'keyword\'=>\''.$unitMonitorInfo['keyword'].'\',\'pathFile\'=>$_SERVER[\'REQUEST_URI\'],\'resultToTest\'=>'.$resultToTestCodePortion.')), CURLOPT_RETURNTRANSFER => true,CURLOPT_ENCODING => "",CURLOPT_MAXREDIRS => 10,CURLOPT_TIMEOUT => 30,CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,CURLOPT_CUSTOMREQUEST => "GET",CURLOPT_HTTPHEADER => array("cache-control: no-cache"),));$response = curl_exec($curl);curl_close($curl);
			');
		echo('</div>');		
	}
