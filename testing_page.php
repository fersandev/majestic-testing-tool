...
// Unit Monitor ----- start
$resultToTest = 7; 
$curl = curl_init();curl_setopt_array($curl, array(CURLOPT_URL => "http://localhost/mtt/majestic-testing-tool/mttphp.php?flag=php&json=".json_encode(array('keyword'=>'la_constitución','pathFile'=>$_SERVER['REQUEST_URI'],'resultToTest'=>!is_string($resultToTest) ? $resultToTest : urlencode($resultToTest))), CURLOPT_RETURNTRANSFER => true,CURLOPT_ENCODING => "",CURLOPT_MAXREDIRS => 10,CURLOPT_TIMEOUT => 30,CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,CURLOPT_CUSTOMREQUEST => "GET",CURLOPT_HTTPHEADER => array("cache-control: no-cache"),));$response = curl_exec($curl);curl_close($curl); // ------------------- end.
